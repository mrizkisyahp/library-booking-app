...
<div class="min-h-screen bg-linear-to-br from-slate-50 to-slate-100">
  <div class="max-w-7xl mx-auto px-6 py-12">
    ...
    <form method="get" action="/rooms" class="bg-white rounded-2xl shadow-lg p-8 mb-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div>
          ...
          <input type="number" name="kapasitas_min" value="<?= htmlspecialchars($filters['kapasitas_min'] ?? '') ?>"
            min="1" placeholder="Misal: 10"
            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all">
        </div>
        ...
        <div>
          ...
          <input type="number" name="kapasitas_max" value="<?= htmlspecialchars($filters['kapasitas_max'] ?? '') ?>"
      ...
      <div class="flex gap-3 items-center justify-end">
        <button type="submit"
          class="bg-primary text-white px-8 py-3 rounded-xl hover:bg-emerald-700 transition-all font-semibold shadow-lg hover:shadow-xl">
        ...
        <?php if (!empty($filters['keyword']) || !empty($filters['kapasitas_min']) || !empty($filters['kapasitas_max']) || !empty($filters['jenis_ruangan'])): ?>
          <a href="/rooms"
            class="bg-gray-200 text-gray-700 px-6 py-3 rounded-xl hover:bg-gray-300 transition-all font-semibold">
            Reset Filter
          </a>
        <?php endif; ?>
      </div>
    ...
    <!-- Room List -->
    <div
      class="room-list <?= (auth()->user()->status === 'pending kubaca' || auth()->user()->status === 'rejected') ? 'opacity-75' : '' ?>">
      <?php if (empty($rooms)): ?>
      ...
      <?php else: ?>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <?php foreach ($rooms as $room): ?>
            ...
            <div
              class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 relative">
              ...
              <div class="flex">
                ...
                <div class="flex-1 p-6">
                  ...
                  <div class="space-y-2 mb-4">
                    ...
                    <?php $facilities = room_facilities($room); ?>
                    <?php if (!empty($facilities)): ?>
                      <div class="flex items-start text-slate-600">
                        <svg class="w-5 h-5 mr-2 mt-0.5 text-emerald-600 shrink-0" fill="none" stroke="currentColor"
                          viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
