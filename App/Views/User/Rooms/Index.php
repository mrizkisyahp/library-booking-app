...
<div class="min-h-screen bg-linear-to-br from-slate-50 to-slate-100">
  <div class="max-w-7xl mx-auto px-6 py-12">
    ...
    <!-- FORM: tempat input#filterToggle (peer), search, tombol filter (label) dan PANEL (sibling) -->
    <form method="get" action="/rooms" class="relative bg-white rounded-2xl shadow-lg p-8 mb-10">
      <?= csrf_field() ?>
      ...
      <div class="relative w-full mb-4">
        ...
        <input type="text" name="nama_ruangan"
               value="<?= htmlspecialchars($filters['nama_ruangan'] ?? '') ?>"
      ...
      <label for="filterToggle"
             class="block w-full cursor-pointer flex items-center justify-center gap-3
      ...
      <div
        class="fixed left-0 right-0 bottom-0 h-[80vh] z-[999] bg-white
        ...
        <div class="space-y-6">
          ...
          <div>
            ...
            <input type="number" name="kapasitas_min" min="0" value="<?= htmlspecialchars($filters['kapasitas_min'] ?? '') ?>"
                   class="w-full mt-1 border-2 border-gray-200 rounded-2xl py-3 px-4 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 focus:outline-none" />
          </div>
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              ...
              ?>
                <label for="<?= $safeId ?>" class="flex items-center bg-slate-100 py-2 px-4 border-2 border-gray-300 rounded-2xl cursor-pointer checked:bg-emerald-600 checked:text-white">
                  <input id="<?= $safeId ?>" type="checkbox" name="jenis_ruangan[]" value="<?= htmlspecialchars($roomType) ?>"
                         class="appearance-none " <?= in_array($roomType, $filters['jenis_ruangan'] ?? []) ? 'checked' : '' ?> />
          ...
          <div class="flex gap-3">
            <button type="submit" class="flex-1 px-4 py-3 bg-emerald-600 text-white rounded-2xl font-semibold hover:bg-emerald-700 transition">
              Terapkan
            </button>

            <a href="/rooms" class="flex-1 px-4 py-3 border-2 border-slate-300 rounded-2xl text-slate-700 text-center hover:bg-slate-100 transition">
              Reset
            </a>
          </div>
        </div>
