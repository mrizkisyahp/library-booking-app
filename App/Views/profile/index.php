...
<?php if ($user): ?>
  ...
  <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100 mb-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <span class="inline-block px-3 py-1 <?= $user->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' ?> text-sm font-semibold rounded-full">
          <?= htmlspecialchars(ucfirst($user->status)) ?>
        </span>
      </div>
      <div>
        <label class="block text-sm font-semibold text-gray-600 mb-1">Peringatan</label>
        <span class="inline-block px-3 py-1 <?= $user->peringatan > 0 ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800' ?> text-sm font-semibold rounded-full">
  ...
  <?php if ($user->isMahasiswa()): ?>
    <?php if ($user->status === 'verified' && !$user->kubaca_img): ?>
      <!-- KuBaca Upload Card -->
    ...
    <?php elseif ($user->kubaca_img && $user->status === 'verified'): ?>
