...
<div class="max-w-7xl mx-auto">  
  ...
  <div class="bg-white rounded-2xl shadow-lg p-8 mb-8">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-3xl font-bold text-slate-800">Tambah Pengguna</h1>
        <p class="text-slate-600 mt-1">Buat akun baru untuk pengguna sistem</p>
      </div>
    </div>
  </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <select id="role" name="role"
            ...
            foreach ($roles as $role): 
                $isSelected = ($role['nama_role'] === $selectedRole) ? 'selected' : '';
            ?>
                <option value="<?= htmlspecialchars($role['nama_role']) ?>" <?= $isSelected ?>>
      ...
      <div class="flex items-center justify-center w-full mb-8">
        <label for="foto_kubaca"
          ...
          <div class="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p class="text-xs text-slate-500 mt-1">PNG, JPG, PDF (max 2MB)</p>
          </div>
          <input id="foto_kubaca" name="foto_kubaca" type="file" class="hidden" accept=".jpg,.jpeg,.png,.pdf">
        </label>
      </div>
