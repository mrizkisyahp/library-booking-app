...
<div class="max-w-5xl mx-auto space-y-6">
  <div class="mb-2">
    <a href="/admin/bookings" class="inline-flex items-center text-emerald-600 hover:text-emerald-700 font-semibold group">
  ...
  <div class="bg-white rounded-2xl shadow-lg p-8">
    <div class="flex items-center justify-between mb-2">
      ...
      <span class="inline-flex px-4 py-2 rounded-lg font-semibold text-sm border <?= $statusColors[$booking->status] ?? 'bg-slate-100 text-slate-700 border-slate-200' ?>">
      ...
      <div class="bg-white rounded-2xl shadow-lg p-8 space-y-5">
        ...
        <div class="space-y-4">
          <div class="p-4 bg-slate-50 rounded-xl">
            ...
            <?php if ($room): ?>
              <p class="text-sm text-slate-600">Kapasitas: <?= htmlspecialchars((string)$room->kapasitas_min) ?> - <?= htmlspecialchars((string)$room->kapasitas_max) ?> orang</p>
            <?php endif; ?>
          </div>
            ...
            <table class="w-full">
              ...
              <tbody class="divide-y divide-slate-100 bg-white">
                <?php foreach ($members as $member): ?>
                  <tr>
                    <td class="px-4 py-3 text-sm font-semibold text-slate-800"><?= htmlspecialchars($member['nama'] ?? '-') ?></td>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 space-y-5">
        ...
        <?php if ($booking->status === 'pending'): ?>
          <form action="/admin/bookings/verify" method="post" class="space-y-3">
            ...
            <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 px-4 rounded-xl transition-colors">
              Konfirmasi Booking
            </button>
          </form>
        <?php elseif ($booking->status === 'verified'): ?>
          <form action="/admin/bookings/activate" method="post" class="space-y-3">
            ...
            <label class="block text-sm font-semibold text-slate-700">
              ...
              <input type="text" name="checkin_code" class="mt-2 w-full border border-slate-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="CT-XXXX" required>
            </label>
        ...
        <div class="pt-4 border-t border-slate-200">
          <form action="/admin/bookings/cancel" method="post" class="space-y-3">
            ...
            <label class="block text-sm font-semibold text-slate-700">
              ...
              <textarea name="reason" rows="3" class="mt-2 w-full border border-slate-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-rose-500" placeholder="Alasan pembatalan (opsional)"></textarea>
            </label>
            <button type="submit" class="w-full bg-rose-600 hover:bg-rose-700 text-white font-semibold py-3 px-4 rounded-xl transition-colors">
              Batalkan Booking
            </button>
          </form>
        </div>
...
<div id="imagePopUp" class="hidden fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-md z-50">
      <div class="bg-white rounded-lg shadow-lg max-w-sm w-full p-4 transition-all scale-95 opacity-0 duration-300">
        ...
        <img id="popUpImage" src="<?= !empty($picKubaca) ? $picKubaca : '' ?> " alt="Pop-up Image" class="w-full h-64 object-cover rounded-md mb-4">
        <div class="flex items-center justify-start gap-4">
          ...
          <p id="popUpId" class="text-sm font-semibold text-gray-600"><?= htmlspecialchars($pic?->nim ?? $pic?->nip ?? '') ?></p>
        </div>
        <div class="flex items-center justify-start gap-4">
          <p>
            Nama:
